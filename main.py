...
def get_descrip(description):
    # soup = re.findall('>\n?(.*)<br/>?<br/>|\n?(.*)</div>|\n(.*)<br/>?<br/>|\n(.*?)\n', str(description))
    soup = re.findall(r'\"description\">(.*)</div>|\n', str(description))
    print("=================================")
    print('soup = ', soup)
    data = ''
    for i in soup:
        for j in i:
            if j != '' and j != '\n' and j != '<br/>' and j != '"description">':
                data += j
    return data


url = 'https://goldapple.ru/19000138441-dylan-purple-pour-femme'
...
print(description_html)
print('description')
description = get_descrip(description_html)
print(description)
print('---------------')
